...
namespace VRStandardAssets.Utils
{
    ...
    public class BarManager : MonoBehaviour
    {
        ...
        private IEnumerator FillBar()
        {
            ...
            if (OnBarFilled != null)
                OnBarFilled();
            //OnBarFilled in separate script subscribing to this behavior
            //this way, 
...
public class BarManager : MonoBehaviour {
    ...
    void Update()
    {
        maxVal = player.GetComponent<PlayerScript>().HealthMax;
        currentVal = Mathf.Clamp(player.GetComponent<PlayerScript>().Health,0, maxVal);
        

        fillAmount = currentVal / maxVal;

      

        if (fillAmount != fillBar.fillAmount)
        {
           
                
            fillBar.fillAmount = fillAmount;

			displayText.text = currentVal + " / " +maxVal;
